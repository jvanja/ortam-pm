...
defineProps<{
  filesData: { name: string; size: number; mime_type: string; path: string }[];
}>();
...
<template>
  <div v-if="filesData.length > 0" class="mt-3 flex flex-col gap-2">
    <div class="w-full">
      ...
      >
        <div class="space-between flex h-full flex-row items-center gap-4">
          <div v-if="file.mime_type === 'application/pdf'" class="h-6 w-6 text-teal-400" v-html="PDF" />
          <FileImage v-else-if="file.mime_type.includes('image')" class="h-6 w-6 text-rose-400" />
          <FileText v-else-if="file.mime_type.includes('text')" class="h-6 w-6 text-indigo-400" />
          <div class="flex flex-col gap-0">
            <div class="text-[0.85rem] font-medium leading-snug">{{ file.name.split('.').slice(0, -1).join('.') }}</div>
            <div class="text-[0.7rem] leading-tight text-gray-500">.{{ file.name.split('.').pop() }} • {{ formatSize(file.size) }}</div>
          </div>
        </div>
        <div>
          <a :href="'/' + file.path" target="_blank" class="text-blue-500 text-sm hover:underline">Download</a>
        </div>
      </div>
    </div>
  </div>
