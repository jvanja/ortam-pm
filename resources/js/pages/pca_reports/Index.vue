...
import { type pca_reportsEntity } from '@/types/DatabaseModels';
import { Head } from '@inertiajs/vue3';

const props = defineProps<{
    pca_reports: pca_reportsEntity[];
}>();
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="flex flex-col gap-4 p-4">
                <Heading title="pca_reports" description="These are your latest pca_reports" />
                <div class="flex flex-col gap-2">
                    <div v-for="pca_report in pca_reports" :key="pca_report.id" class="flex justify-between rounded-lg bg-neutral-100 p-1 dark:bg-neutral-800">
                        <div class="px-4 py-2 text-sm font-medium">{{ pca_report.pca_name }}</div>
