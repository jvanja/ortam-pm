...
const modelChanged = (modelValue: AcceptableValue) => {
  if (typeof modelValue === 'string' && modelValue.split('|')[0] === 'new') {
    ...
    router.get(route(`${model}.create`) + `?${model}=true`);
  }
};
</script>
    ...
    <div class="p-8">
      ...
      <form @submit.prevent="submit" class="space-y-6 rounded border bg-card p-6 text-card-foreground shadow">
        ...
        <div class="mb-4">
          ...
          <Select id="client_id" v-model="form.client_id" @update:modelValue="modelChanged" required>
            <SelectTrigger>
              ...
              <SelectGroup>
                ...
                <SelectItem value="new|clients" class=" bg-teal-50 p-1 px-4 py-2 dark:bg-neutral-800">
                  <div class="flex items-center gap-2">
                    <PlusCircle class="text-green-700" />Add new client
                  </div>
                </SelectItem>
              </SelectGroup>
            </SelectContent>
            ...
            <SelectContent>
              <SelectGroup>
                ...
                <SelectItem value="new|projects" class=" bg-teal-50 p-1 px-4 py-2 dark:bg-neutral-800">
                  <div class="flex items-center gap-2">
                    <PlusCircle class="text-green-700" />Add new project
                  </div>
                </SelectItem>
              </SelectGroup>
            </SelectContent>
